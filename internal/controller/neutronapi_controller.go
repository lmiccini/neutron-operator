...
func (r *NeutronAPIReconciler) reconcileDelete(ctx context.Context, instance *neutronv1beta1.NeutronAPI, helper *helper.Helper) (ctrl.Result, error) {
	...
	// Remove transport URL consumer finalizer
	if err := object.RemoveSecretConsumerFinalizer(ctx, helper, instance.Namespace,
		instance.Status.TransportURLSecret, neutronapi.TransportConsumerFinalizer); err != nil {
		return ctrl.Result{}, err
	}

	// Remove notification transport URL consumer finalizer
...
func (r *NeutronAPIReconciler) reconcileNormal(ctx context.Context, instance *neutronv1beta1.NeutronAPI, helper *helper.Helper) (ctrl.Result, error) {
	...
	if instance.Spec.NotificationsBus != nil && instance.Spec.NotificationsBus.Cluster != "" {
		...
		secretVars[currentTransportSecret] = env.SetValue(notificationsTransportURLSecretHash)
	...
	// No sub-CRs — neutron directly manages Deployments, so pass true for stability.
	...
	// Manage the old AC secret's finalizer and status tracking using the same
	// guardReady variable. On rotation (old != new), only remove the old
	// secret's finalizer after all sub-services are ready with the new credentials.
